...
function showResult(data){
    ...
    for (let i = 0;i<data.coins.length;i++) {
     ...
     new_a.href = `details.html?coin=${single_data.id}`  ; 
...
let search_coin = params.get("q");

search_field.value = search_coin; 
fetch(`https://api.coingecko.com/api/v3/search?query=${search_coin}`).then (convertToJson).then (showResult);
